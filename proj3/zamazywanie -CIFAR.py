...
def get_dataset(trans):
    ...
    test_dataset = datasets.CIFAR10(root='./data', train=False, download=True, transform=transforms_no_aug)
...
def coverup(inp: Tensor, std: float) -> Tensor:
    new_inp = inp
    for i in range(std):
        new_inp[0, :, randint(0, 27), randint(0, 27)] = 0
    return new_inp

def fill_in(inp: Tensor, std: float) -> Tensor:
    new_inp = inp
    for i in range(std):
        new_inp[0, :, randint(0, 27), randint(0, 27)] = 255
    return new_inp
            ...
            #for train_dataset in train_dataset_table:
                ...
                for feature_size in out_features:
                    ...
                    for images, labels in test_loader:
                        oryg_img = images[0].numpy().copy()
                        plt.imshow(zxy_to_xyz(oryg_img))
                        ...
                        with torch.no_grad():
                            ...
                            if (predicted != predicted2):
                                print("Real label: " + str(int(labels[0])) + " No modifications label: " + str(int(predicted[0])) + " With modifications label: " + str(int(predicted2[0])))
                                plt.show()
                                print(noised_image)
                                plt.imshow(zxy_to_xyz(noised_image[0].numpy()))
